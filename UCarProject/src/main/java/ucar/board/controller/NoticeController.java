...
public class NoticeController {
	...
	/**
	 * 고객센터 누르면 가장 먼저 뜨는 페이지
	 * 최근 게시물 5개를 보여주는 메서드
	 */
	@RequestMapping("customercenter_home_notice.do")
	public ModelAndView list(String pageNo) {	
		return new ModelAndView("customercenter_notice_form","lvo",boardService.getBoardList(pageNo));
	}
	/**
	 * 게시글 조회,
	 * 게시글 클릭시 세부 내용을 볼 수있는 폼으로 이동
	 * @return
	 * @
	 */
	...
	public ModelAndView showContent(int noticeNo) {		
		...
		return new ModelAndView("customercenter_notice_showcontent_form","bvo",bvo);
	}
	/**
	 * 관리자가 게시글 삭제
	 * @param bvo
	...
	public ModelAndView updateBoard(BoardVO bvo) {
		...
		return new ModelAndView("redirect:notice_showContent.do?noticeNo="+bvo.getNoticeNo());
	}
