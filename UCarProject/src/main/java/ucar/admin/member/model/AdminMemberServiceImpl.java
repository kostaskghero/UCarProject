...
public class AdminMemberServiceImpl implements AdminMemberService {
	...
	/**
	 * 아이디로 해당 회원의 상세정보를 반환하는 메서드 해당아이디에 면허정보와 
	 * 카드정보들이 등록되어있는지 확인하여 MemberVO에
	 * 넣어준뒤 면허와 카드 모두 등록한경우, 카드만 등록한 경우, 면허만 등록한경우,
	 * 둘다 등록하지 않은 경우의 4가지 경우의 수에 맞게
	 * 결과 리스트를 반환한다.
	 */
	@Override
	...
	/**
	 * 트랜젝션을 이용하여 회원을 탈퇴시킬때 해당아이디의 카드정보와 면허정보를 함께 지워준다.
	 * DB에서 on Delete cascade를
	 * 명시한 해당아이디의 참조테이블들이 함께 삭제 된다.
	 */
	@Transactional
	@Override
	...
	/**
	 * 전체 포인트 내역을 반환하는 메서드 페이징 처리를 위해 해당 페이지와 그 리스트를 listVO에 담아 반환한다.
	 */
	@Override
	...
	/**
	 * 검색한 아이디에 해당하는 포인트 내역을
	 * 페이징빈과 함께listVO에 담아 반화하는 메서드
	 */
	@Override
	public PointListVO getAllPointHistoryById(String memberId, String pageNo) {
		if (pageNo == null || pageNo == "")
			pageNo = "1";
		List<PointHistoryVO> list = adminMemberDAO.getAllPointHistoryById(
				memberId, pageNo);
	...
	/**
	 * 트랙젝션으로 관리자가 해당회원에게 포인트를 지급하는 메서드.
	 *  포인트 내역테이블과 해당 아이디의 member테이블에 모두 포인트를
	 * 업데이트 해준다.
	 */
	@Override
	@Transactional
